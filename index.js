...
function runquery(feature,response){
    var query = "select DISTINCT b.NAME, b.ADDRESS, b.STARS, b.REVIEW_COUNT from BUSINESS b natural join CATEGORIES c WHERE (b.LAT -(" + lat + ")) * (b.LAT -(" + lat + ")) + (b.LON -(" + long + ")) * (b.LON -(" + long + ")) < 3 AND c.FEATURE = '" + feature + "'";
...
app.get('/search_pop/:city/:feature', function(request, response){
    ...
    console.log(city)
    var feature = request.params.feature;
    var query = "select DISTINCT b.NAME, b.ADDRESS, b.STARS, b.REVIEW_COUNT from BUSINESS b natural join CATEGORIES c WHERE c.FEATURE = '" + feature + "' AND b.CITY = '" + city + "' AND b.REVIEW_COUNT > 100 AND b.STARS = 5"
    console.log(query)
    database.execute(query, function(error, result) {
        if (error) {
            throw error
